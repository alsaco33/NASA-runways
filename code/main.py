...
def main(data_path, models_path, build_master=False):
    ...
    if build_master:
        ...
        master_table = CreateMaster(data_path=data_path, 
                                    airports=AIRPORTS,
    ...
    # Build catboost models at the timestamp-airport level for each airport and timestamp
    for airport in AIRPORTS:
        for target in [f'target_{l}' for l in LOOKAHEADS]:
        
            # Filter to stick only with relevant rows
            current = master_table[(master_table[target].isna()==False) & 
                                   (master_table.airport == airport) &
                                   (master_table.day.isin(master_table[master_table.in_test.isna()==False]['day'].unique())==False)
            ...
            for eta in [0.005, 0.01, 0.02, 0.03, 0.05]:
                for depth in [3, 4, 5, 6, 7]:
                    ...
                    model = CatBoostClassifier(eta=eta,
                                               n_estimators=6000,
                                               task_type='CPU',
                                               thread_count=-1,
                                               depth=depth,
                    ...
                    print('@@@@@@@@@@@@@@@', airport, target, eta, depth, model.best_score_['validation']['MultiClass'])
